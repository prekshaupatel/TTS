...
class TextToMel:
    ...
    def load_glow_tts(self):
        ...
        checkpoint_path = '../../checkpoints/glow/male/G_400.pth' # utils.latest_checkpoint_path(self.glow_model_dir)
...
class MelToWav:
    ...
    def load_hifi_gan(self):
        checkpoint_path = '../../checkpoints/hifi/male/g_00060000' # utils.latest_checkpoint_path(self.hifi_model_dir, regex="g_*")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--textfile", type=str, required=True)
    parser.add_argument("-w", "--wav", type=str, required=True)
    parser.add_argument("-s", "--timestamp", type=int, required=True)
    parser.add_argument("-f", "--labelfile", type=str, required=True)
    ...
    labelfile = open(os.path.join(args.wav, args.labelfile), 'w')

    datafile = open(args.textfile)
    texts = datafile.read().splitlines()

    timestamp = args.timestamp
    for text in texts:
        timestamp = timestamp + 1
        mel = text_to_mel.generate_mel(text, args.noise_scale, args.length_scale)
        audio, sr = mel_to_wav.generate_wav(mel)
        
        write(filename=os.path.join(args.wav, 'wav', '%d.wav' % timestamp), rate=sr, data=audio)
        labelfile.write('%d.wav\t%s\n' % (timestamp, text))
